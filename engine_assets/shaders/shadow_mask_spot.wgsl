// Screen-space shadow mask for one spot light: projects the spot's single
// shadow map (the shared, serially-reused spot depth tile) against the scene
// depth with a 3x3 PCF.  Writes the factor (1 = lit) to the per-light temp
// mask (target 0, replace) and multiplies it into the frame's accumulation
// texture (target 1, multiply blend) for the later Gaussian overlay.
