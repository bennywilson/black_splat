// Screen-space shadow mask for the cascaded directional light: for every
// pixel with world geometry, picks the tightest cascade containing it and
// PCF-samples the cascade atlas.  Writes the factor (1 = lit) to the
// per-light temp mask (target 0, replace) and multiplies it into the frame's
// accumulation texture (target 1, multiply blend) for the later Gaussian
// overlay.  Pixels without geometry (background / splats-only) stay 1.
